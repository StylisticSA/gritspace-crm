...
const deleteextra = () => {
    if (amenityToDelete.value) {
        router.delete(route('admin.extra.destroy', amenityToDelete.value), {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    <Head title="Extra settings" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Extra Setting</h2>
        </template>
                ...
                <div class="p-2">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full border border-gray-300 divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr
                                    v-for="cofe in extra.data"
                                    :key="cofe.id">
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ cofe.id }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ cofe.location?.name }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ cofe.name }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ cofe.price }}</td>
                    ...
                    <div class="flex items-center justify-between mt-4">
                        <div class="text-sm text-gray-600">
                            Showing
                            <span class="font-medium">{{ extra.from }}</span>
                            to
                            <span class="font-medium">{{ extra.to }}</span>
                            of
                            <span class="font-medium">{{ extra.total }}</span> results
                        </div>

                        <div class="flex space-x-1">
                            <template
                                v-for="(link, index) in extra.links"
                                :key="index">
                                <Link
                                    v-if="link.url"
                                    :href="link.url"
                ...
                <template v-if="showModal">
                    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                        <div class="w-full max-w-md p-6 bg-white rounded shadow">
                            ...
                            <p class="mb-6">
                                Are you sure you want to delete this Extra settings? This action cannot be undone.
                            </p>
