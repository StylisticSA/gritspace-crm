...
const confirmDelete = id => {
    ...
    printToDelete.value = id;
};

const deleteamenity = () => {
    if (printToDelete.value) {
        router.delete(route('admin.printing.destroy', printToDelete.value), {
            preserveScroll: true,
            onSuccess: () => {
                window.scrollTo({ top: 0, behavior: 'smooth' });
            },
                ...
                <div class="p-2">
                        ...
                        <table class="min-w-full border border-gray-300 divide-y divide-gray-200">
                            <thead class="bg-gray-100">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-sm font-medium text-left text-gray-700">Color Total</th>
                                    <th class="px-6 py-3 text-sm font-medium text-left text-gray-700">Black Total</th>
                                    <th class="px-6 py-3 text-sm font-medium text-left text-gray-700">Taken On</th>
                                    <th class="px-6 py-3 text-sm font-medium text-left text-gray-700">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr
                                    v-for="cofe in printing.data"
                                    :key="cofe.id">
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ cofe.id }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ cofe.user.name }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ cofe.location?.name }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        {{ cofe.color_total_cost ?? 'None' }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        {{ cofe.black_total_cost ?? 'None' }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ formatDate(cofe.date) }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        <div class="flex space-x-1">
                                            <button
                                                v-if="can['manage settings']"
                                                @click="$inertia.visit(route('printing.edit', cofe.id))"
                    ...
                    <div class="flex items-center justify-between mt-4">
                        <div class="text-sm text-gray-600">
                            Showing
                            <span class="font-medium">{{ printing.from }}</span>
                            to
                            <span class="font-medium">{{ printing.to }}</span>
                            of
                            <span class="font-medium">{{ printing.total }}</span> results
                        </div>

                        <div class="flex space-x-1">
                            <template
                                v-for="(link, index) in printing.links"
                                :key="index">
                                <Link
                                    v-if="link.url"
                                    :href="link.url"
