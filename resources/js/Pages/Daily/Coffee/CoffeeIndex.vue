...
const confirmDelete = id => {
    ...
    CoffeeDelete.value = id;
};

const deleteamenity = () => {
    if (CoffeeDelete.value) {
        router.delete(route('admin.coffee.destroy', CoffeeDelete.value), {
            preserveScroll: true,
            onSuccess: () => {
                window.scrollTo({ top: 0, behavior: 'smooth' });
            },
...
<template>
    <Head title="Coffee Admin" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Coffee's</h2>
        </template>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="p-2">
                    ...
                    <div class="flex flex-col gap-3 mb-4 sm:flex-row sm:items-center sm:justify-between">
                        <Link
                            v-if="can['create coffee']"
                            :href="route('admin.coffee.create')"
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        <div class="flex space-x-1">
                                            <button
                                                ...
                                                @click="$inertia.visit(route('coffee.edit', cofe.id))"
                    ...
                    <div class="flex items-center justify-between mt-4">
                        <div class="text-sm text-gray-600">
                            Showing
                            <span class="font-medium">{{ coffee.from }}</span>
                            to
                            <span class="font-medium">{{ coffee.to }}</span>
                            of
                            <span class="font-medium">{{ coffee.total }}</span> results
                        </div>

                        <div class="flex space-x-1">
                            <template
                                v-for="(link, index) in coffee.links"
                                :key="index">
                                <Link
                                    v-if="link.url"
                                    :href="link.url"
