...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Booked Virtual Office</h2>
        </template>
                ...
                <div class="flex justify-center">
                    <div class="w-full max-w-2xl p-6 space-y-6 bg-white border rounded-lg shadow-md">
                        ...
                        <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                            <h3 class="text-xl font-semibold">{{ virtual?.virtual_office?.virtualoffice_name }}</h3>
                            <Link
                        ...
                        <div class="grid grid-cols-1 gap-6 pt-5 text-sm text-gray-700 md:grid-cols-2">
                            <div class="space-y-2">
                                <div class="grid grid-cols-[140px_1fr] gap-x-2 items-start">
                                    ...
                                    <div>{{ virtual?.user?.name }}</div>

                                    <div class="mb-3 font-medium text-gray-600">
                                        <strong>Location: </strong>
                                    </div>
                                    <div class="mb-2">{{ virtual?.virtual_office?.location?.name }}</div>

                                    <div class="mb-3 font-medium text-gray-600">
                                        <strong>Plan: </strong>
                                    </div>
                                    <div class="mb-2">{{ virtual?.plan }}</div>

                                    <div class="mb-3 font-medium text-gray-600">
                                        <strong>Duration: </strong>
                                    </div>
                                    <div class="mb-2">{{ virtual?.months }}</div>
                                </div>
                            </div>
                            ...
                            <div>
                                <div class="grid grid-cols-[140px_1fr] gap-x-2 items-start">
                                    ...
                                    <div class="mb-2">
                                        <span
                                            :class="{
                                                ...
                                                'bg-yellow-100 text-yellow-800': virtual?.status === 'pending',
                                                'bg-green-100 text-green-800': virtual?.status === 'approved',
                                                'bg-gray-200 text-gray-700': virtual?.status === 'cancelled',
                                                'bg-red-100 text-red-700': virtual?.status === 'rejected',
                                            }">
                                            {{ virtual?.status ?? 'N/A' }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="mb-2">R{{ virtual?.selected_price }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
