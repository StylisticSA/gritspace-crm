...
const uniqueVirtualOffices = computed(() => {
    const seen = new Map();
    props.events.forEach(event => {
        const id = event.extendedProps?.virtual_office_id;
        const name = event.extendedProps?.virtual_office;
...
watchEffect(() => {
    activeFilters.value = uniqueVirtualOffices.value.map(v => v.id);
...
const filteredEvents = computed(() =>
    props.events.filter(
        event =>
            activeFilters.value.includes(event.extendedProps?.virtual_office_id) &&
...
const calendarOptions = ref({
    ...
    eventContent: function (arg) {
        ...
        const office = arg.event.extendedProps?.virtual_office;
    ...
    eventClick: info => {
        ...
        selectedEvent.value = {
            ...
            office: info.event.extendedProps?.virtual_office,
...
const groupedVirtualOffices = computed(() => {
    const groups = {};
    props.events.forEach(event => {
        const locName = event.extendedProps?.location;
        const officeId = event.extendedProps?.virtual_office_id;
        const officeName = event.extendedProps?.virtual_office;
        ...
        <div class="py-12">
            <div class="flex flex-col gap-6 px-4 mx-auto lg:flex-row max-w-7xl lg:px-8">
                ...
                <aside
                    ...
                    <h3 class="mb-4 text-lg font-semibold">Filter Virtual Offices</h3>

                    <div class="mb-2">
                        <button
                            @click="activeFilters = uniqueVirtualOffices.map(v => v.id)"
                    ...
                    <div
                        v-for="(offices, location) in groupedVirtualOffices"
                        :key="location">
                        <button
